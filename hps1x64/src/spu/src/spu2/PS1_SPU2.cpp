...
static SPUCore::Regs SPUCore::Regs16;
...
static u64* SPUCore::_SystemCycleCount;
...
static bool SPUCore::DebugWindow_Enabled [ 2 ];
static WindowClass::Window *SPUCore::DebugWindow [ 2 ];
static DebugValueList<u16> *SPUCore::SPUMaster_ValueList [ 2 ];
static DebugValueList<u16> *SPUCore::SPU_ValueList [ 24 ] [ 2 ];
static Debug_MemoryViewer *SPUCore::SoundRAM_Viewer [ 2 ];
...
//static SPUCore::SpuRegs0_t *SPUCore::SpuRegs0;
//static SPUCore::SpuRegs1_t *SPUCore::SpuRegs1;
...
void SPUCore::Write ( u32 Offset, u32 Data, u32 Mask )
{
	...
	switch ( ROffset )
	{
		...
		case RVWAS_1:
			...
			if ( ( Reverb_BufferAddress == ( SWAPH ( pCoreRegs0->IRQA ) ) ) && ( pCoreRegs0->CTRL & 0x40 ) )
			{
...
void SPU2::Run ()
{
	...
	*/
	...
	if ( Dma::_DMA->isEnabledAndActive ( 7 ) )
	{
		...
		if ( ( SPU1.DecodeBufferOffset & 0x1ff ) == 256 && SPU1.isADMATransferMode () )
		{
#ifdef INLINE_DEBUG_SPU2RUN
	debug << "\r\nDMA7 ADMA:";
	...
	{
		// check if in ADMA transfer mode
		if ( ( SPU0.DecodeBufferOffset & 0x1ff ) == 256 && SPU0.isADMATransferMode () )
		{
#ifdef INLINE_DEBUG_SPU2RUN
	debug << "\r\nDMA4 ADMA:";
...
//#endif
	...
	if ( pCoreRegs0->CTRL >> 15 )
	{
		...
		if ( pCoreRegs1->MVOL_L >> 15 )
		{
			...
			VolumeEnvelope ( pCoreRegs1->CMVOL_L, MVOL_L_Cycles, pCoreRegs1->MVOL_L & 0x7f, ( pCoreRegs1->MVOL_L >> 13 ) & 0x3 );
		}
		else
		{
		...
		if ( pCoreRegs1->MVOL_R >> 15 )
		{
			...
			VolumeEnvelope ( pCoreRegs1->CMVOL_R, MVOL_R_Cycles, pCoreRegs1->MVOL_R & 0x7f, ( pCoreRegs1->MVOL_R >> 13 ) & 0x3 );
		}
		else
		{
		...
		*/
		...
		//if ( Regs [ ( 0x1f801daa - SPU_X ) >> 1 ] >> 15 )
			...
			if ( pChRegs0->VOL_L >> 15 )
			{
				...
				VolumeEnvelope ( pChRegs0->CVOL_L, VOL_L_Cycles [ Channel ], pChRegs0->VOL_L & 0x7f, ( pChRegs0->VOL_L >> 13 ) & 0x3 );
			...
			if ( pChRegs0->VOL_R >> 15 )
			{
				...
				VolumeEnvelope ( pChRegs0->CVOL_R, VOL_R_Cycles [ Channel ], pChRegs0->VOL_R & 0x7f, ( pChRegs0->VOL_R >> 13 ) & 0x3 );
	...
	debug << "; (before) VOL_ADSR_Value=" << hex << pChRegs0->ENV_X << "; Cycles=" << dec << Cycles [ Channel ];
					...
					VolumeEnvelope ( (s16&) pChRegs0->ENV_X, Cycles [ Channel ], ( ModeRate >> 8 ) & 0x7f, ( ModeRate >> 15 ) << 1 );
	...
	debug << "; DECAY_NEXT";
						...
						Start_VolumeEnvelope ( (s16&) pChRegs0->ENV_X, Cycles [ Channel ], ( ( ModeRate >> 4 ) & 0xf ) << ( 2 ), 0x3 );
					}
	...
	debug << "; (before) VOL_ADSR_Value=" << hex << pChRegs0->ENV_X << "; Cycles=" << dec << Cycles [ Channel ];
					...
					VolumeEnvelope ( (s16&) pChRegs0->ENV_X, Cycles [ Channel ], ( ( ModeRate >> 4 ) & 0xf ) << ( 2 ), 0x3 );
	...
	debug << "; (after) VOL_ADSR_Value=" << hex << pChRegs0->ENV_X << "; Cycles=" << dec << Cycles [ Channel ] << "; Value=" << hex << ( ( ModeRate >> 4 ) & 0xf ) << "; flags=" << ( ( ModeRate >> 15 ) << 1 );
					...
					if ( pChRegs0->ENV_X <= VOL_SUSTAIN_Level [ Channel ] )
					{
#ifdef INLINE_DEBUG_RUN_DECAY
	debug << "; SUSTAIN_NEXT";
#endif
						...
						Start_VolumeEnvelope ( (s16&) pChRegs0->ENV_X, Cycles [ Channel ], ( ModeRate >> 6 ) & 0x7f, ModeRate >> 14 );
					}
	...
	debug << "; (before) VOL_ADSR_Value=" << hex << pChRegs0->ENV_X << "; Cycles=" << dec << Cycles [ Channel ];
					...
					VolumeEnvelope ( (s16&) pChRegs0->ENV_X, Cycles [ Channel ], ( ModeRate >> 6 ) & 0x7f, ModeRate >> 14 );
	...
	debug << "; (before) VOL_ADSR_Value=" << hex << pChRegs0->ENV_X << "; Cycles=" << dec << Cycles [ Channel ];
					...
					else
					{
						...
						VolumeEnvelope ( (s16&) pChRegs0->ENV_X, Cycles [ Channel ], ( ModeRate & 0x1f ) << ( 2 ), ( ( ( ModeRate >> 5 ) & 1 ) << 1 ) | 0x1 );
					}
	...
	debug << "; (after) VOL_ADSR_Value=" << hex << pChRegs0->ENV_X << "; Cycles=" << dec << Cycles [ Channel ] << "; Value=" << hex << ( ( ModeRate >> 8 ) & 0x7f ) << "; flags=" << ( ( ModeRate >> 15 ) << 1 );
			...
			if ( ( ( SWAPH ( pCoreRegs0->IRQA ) ) == ( DecodeBufferOffset + 0x800 ) ) && ( pCoreRegs0->CTRL & 0x40 ) )
			{
				// we have reached irq address - trigger interrupt
				SetInterrupt ();
			...
			if ( ( ( SWAPH ( pCoreRegs0->IRQA ) ) == ( DecodeBufferOffset + 0xc00 ) ) && ( pCoreRegs0->CTRL & 0x40 ) )
			{
				// we have reached irq address - trigger interrupt
				SetInterrupt ();
			...
			/*
			if ( Channel == 3 && CoreNumber == 1 )
			{
				if ( Debug_CurrentSample [ 3 ] )
				{
					cout << " " << hex << Debug_CurrentSample [ 3 ];
				}
			}
			*/
			...
			if ( CurrentSample_Offset [ Channel ] >= ( 28ULL << 32 ) )
			{
				...
				if ( ( ( CurrentBlockAddress [ Channel ] ) == ( SWAPH( pCoreRegs0->IRQA ) ) ) && ( pCoreRegs0->CTRL & 0x40 ) )
				{
					// we have reached irq address - trigger interrupt
					SetInterrupt ();
	...
	*/
	...
	if ( TempL != 0 || TempR != 0 )
	{
		debug << "\r\nMixing CD; SampleL=" << hex << TempL << " SampleR=" << TempR;
			...
			*/
			...
			{
				if ( ( SWAPH( pCoreRegs0->IRQA ) ) == ( DecodeBufferOffset + 0x000 ) )
				{
					// we have reached irq address - trigger interrupt
					SetInterrupt ();
				...
				if ( ( SWAPH( pCoreRegs0->IRQA ) ) == ( DecodeBufferOffset + 0x400 ) )
				{
					// we have reached irq address - trigger interrupt
					SetInterrupt ();
	...
	// check if should mix in the sound data input area data (ADMA)
	if ( pCoreRegs0->MMIX & 0x40 )
	{
		// mix in the right sample
		SampleR += ( ( ( (s64) pCoreSoundDataInputR [ DecodeBufferOffset >> 1 ] ) * ( (s64) pCoreRegs1->BVOL_R ) ) >> c_iVolumeShift );
	}
	
	if ( pCoreRegs0->MMIX & 0x80 )
	{
		// mix in the left sample
		SampleL += ( ( ( (s64) pCoreSoundDataInputL [ DecodeBufferOffset >> 1 ] ) * ( (s64) pCoreRegs1->BVOL_L ) ) >> c_iVolumeShift );
	}
...
void SPUCore::DMA_Read_Block ( u32* Data, u32 BS )
{
	...
	for ( int i = 0; i < BS; i++ )
	{
		...
		if ( ( NextSoundBufferAddress == ( SWAPH ( pCoreRegs0->IRQA ) ) ) && ( pCoreRegs0->CTRL & 0x40 ) )
		{
			// we have reached irq address - trigger interrupt
			SetInterrupt ();
...
static u64 SPU2::DMA_ReadyForRead_Core0 ()
{
	return _SPU2->SPU0.DMA_ReadyForRead ();
}

static u64 SPU2::DMA_ReadyForRead_Core1 ()
{
	return _SPU2->SPU1.DMA_ReadyForRead ();
}

static u64 SPU2::DMA_ReadyForWrite_Core0 ()
{
	return _SPU2->SPU0.DMA_ReadyForWrite ();
}

static u64 SPU2::DMA_ReadyForWrite_Core1 ()
{
	return _SPU2->SPU1.DMA_ReadyForWrite ();
}


static u32 SPU2::DMA_ReadBlock_Core0 ( u32* pMemoryPtr, u32 Address, u32 WordCount )
{
	u32 *Data;
...
static u32 SPU2::DMA_ReadBlock_Core1 ( u32* pMemoryPtr, u32 Address, u32 WordCount )
{
	u32 *Data;
...
static u32 SPU2::DMA_WriteBlock_Core0 ( u32* pMemoryPtr, u32 Address, u32 WordCount )
{
	u32 *Data;
	u32 WC;
...
static u32 SPU2::DMA_WriteBlock_Core1 ( u32* pMemoryPtr, u32 Address, u32 WordCount )
{
	u32 *Data;
	u32 WC;
...
u32 SPUCore::DMA_Write_Block ( u32* Data, u32 BS )
{
	...
	if ( isADMATransferMode () )
	{
		...
		//debug << " BA7=" << hex << Dma::_DMA->DmaCh [ 7 ].BCR.BA;
		debug << " ADMAOffset=" << dec << SoundDataInput_Offset;
		debug << " Cycle#" << dec << *_DebugCycleCount;
		...
		/*
		if ( DecodeBufferOffset < 512 )
		{
			ulBufferHalf_Offset = 0;
		}
		else
		{
		...
		*/
		...
		for ( int i = 0; i < (BS << 1); i++ )
		{
			...
			/*
			if ( ( ( ( NextSoundBufferAddress + i ) & ( c_iRam_Mask >> 1 ) ) == ( SWAPH ( pCoreRegs0->IRQA ) ) ) && ( pCoreRegs0->CTRL & 0x40 ) )
			{
				// we have reached irq address - trigger interrupt
				SetInterrupt ();
...
#ifdef INLINE_DEBUG_DMA_WRITE
		debug << " (after)ADMAOffset=" << dec << SoundDataInput_Offset;
		debug << " ***\r\n";
#endif
		...
		/*
		cout << "\nhps2x64: SPU2: ALERT: AutoDMA transfer! Core#" << CoreNumber;
		
		// interrupt ???
		//SPU2::SetInterrupt_AutoDMA ();
		
		// doesn't have to actually transfer the data for testing
		
		// determine what core data is being transferred for
		ulCoreOffset16 = CoreNumber << 10;
		
		
		// check buffer half to transfer into
		//if ( ulADMA_Offset8
		
		// determine current transfer offset
		ulOffset16 += ( ( ulADMA_Offset8 & 0x1ff ) >> 1 );
		
		// next sound buffer address is supposed to be in halfwords
		//NextSoundBufferAddress >>= 1;
		
		// transfer 256 samples for L (512 bytes)
		memcpy ( & ( RAM [ ulOffset16 & ( c_iRam_Mask >> 1 ) ] ), Data, 512 );
		
		// update offset
		
		
		// transfer 256 samples for R (512 bytes)
		
		// update offset8 for ADMA
		
		// done
		
		for ( int i = 0; i < (BS << 1); i++ )
		{
			// check what buffer to transfer into (left or right samples) (512 samples per buffer/1024 bytes)
			if ( ulADMA_Offset8 & 0x200 )
			{
				// transfer into R
				ulOffset16 = ulCoreOffset16 + 0x2200;
			}
			else
			{
				// transfer into L
				ulOffset16 = ulCoreOffset16 + 0x2000;
			}

			// write the data into sound RAM
			RAM [ ( ulOffset16 + i ) & ( c_iRam_Mask >> 1 ) ] = ((u16*) Data) [ i ];
			
			// check for interrupt
			// check if address is set to trigger interrupt
			if ( ( ( ( ulOffset16 + i ) & ( c_iRam_Mask >> 1 ) ) == ( SWAPH ( pCoreRegs0->IRQA ) ) ) && ( pCoreRegs0->CTRL & 0x40 ) )
			{
				// we have reached irq address - trigger interrupt
				SetInterrupt ();
		...
		// get next transfer offset (BS is number of 32-bit words, so advance BS*4 bytes)
		ulADMA_Offset8 += ( BS << 2 );
		*/
...
void SPUCore::Start_SampleDecoding ( u32 Channel )
{
	...
	*/
	...
	// set start address
	CurrentBlockAddress [ Channel ] = SWAPH ( pChRegs1->SSA );
	...
	if ( ( ( CurrentBlockAddress [ Channel ] ) == ( SWAPH ( pCoreRegs0->IRQA ) ) ) && ( pCoreRegs0->CTRL & 0x40 ) )
	{
		// ***TODO*** don't interrupt if irq info not clear ??
		//if ( !( GET_REG16 ( SPDIF_IRQINFO ) & ( 0x4 << Core ) ) )
		//{
			// we have reached irq address - trigger interrupt
			SetInterrupt ();
...
static void SPUCore::DebugWindow_Enable ( int Number )
{
	...
	static const int SPUMasterList_Width = 100;
	static const int SPUMasterList_Height = 130;
	...
	if ( !DebugWindow_Enabled [ Number ] )
	{
		...
		SPUMaster_ValueList [ Number ]->AddVariable ( "MVOL_L", & pCoreRegs1->MVOL_L );
		SPUMaster_ValueList [ Number ]->AddVariable ( "MVOL_R", & pCoreRegs1->MVOL_R );
		...
		for ( i = 0; i < NumberOfChannels; i++ )
		{
			...
			SPU_ValueList [ i ] [ Number ]->AddVariable ( ss.str().c_str(), & pChRegs0->VOL_L );
			
			ss.str ("");
			ss << c_sChannelStr << i << "_VOLR";
			//SPU_ValueList [ i ] [ Number ]->AddVariable ( ss.str().c_str(), &(_SPU->Regs [ ( ( i << 4 ) + SPU::VOL_L ) >> 1 ]) );
			SPU_ValueList [ i ] [ Number ]->AddVariable ( ss.str().c_str(), & pChRegs0->VOL_R );
			...
			SPU_ValueList [ i ] [ Number ]->AddVariable ( ss.str().c_str(), &(_SPU->Debug_CurrentRawSample [ i ]) );
		...
		SoundRAM_Viewer [ Number ]->Add_MemoryDevice ( "SoundRAM", 0, c_iRam_Size, (char*) _SPU->RAM );
...
static void SPUCore::DebugWindow_Disable ( int Number )
{
...
static void SPUCore::DebugWindow_Update ( int Number )
{
