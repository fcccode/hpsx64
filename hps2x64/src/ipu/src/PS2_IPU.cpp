...
static const s32 IPU::sDitherArray_4_1 [ 4 * 4 ] = { -4 << 1, 0, -3 << 1, 1 << 1,
													2 << 1, -2 << 1, 3 << 1, -1 << 1,
													-5, 3, -7, 1,
													7, -1, 5, -3 };
...
static const char* IPU::RegNames [ 4 ] = { "CMD", "CTRL", "BP", "TOP" };
...
void IPU::Run ()
{
	...
	// how this was done on the ps1 was processing took place when data was requested
	// for now, that should work ok
	
	/*
	switch ( CurrentOp )
	{
		case CURRENTOP_OUTPUT:
			// send data out via dma
			Dma::_DMA->Transfer ( 3 );
			break;
			
		case CURRENTOP_INPUT:
...
void IPU::Process_CMD ()
{
	...
	debug << " State=" << dec << CommandState;
	debug << " " << dec << ipu_cmd.pos [ 0 ];
	debug << " MBC=" << dec << MacroBlock_Count;
#endif
...
// dma#4
static bool IPU::DMA_Write_Ready ()
{
...
// dma#3
static bool IPU::DMA_Read_Ready ()
{
#ifdef INLINE_DEBUG_DMA_WRITE
#endif
...
// dma#4 - toIPU
static u32 IPU::DMA_WriteBlock ( u64* Data, u32 QuadwordCount )
{
#ifdef INLINE_DEBUG_DMA_WRITE
...
// dma#3 - fromIPU
static u32 IPU::DMA_ReadBlock ( u64* Data, u32 QuadwordCount )
{
...
static u64 IPU::Read ( u32 Address, u64 Mask )
{
...
static void IPU::Write ( u32 Address, u64 Data, u64 Mask )
{
