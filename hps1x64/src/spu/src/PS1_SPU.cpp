...
static SPU::CoreRegs_Layout *SPU::pCoreRegs;
...
void SPU::UpdatePitch ( int Channel, u32 Pitch, u32 Reg_PMON, ssX PreviousSample )
{
	s64 Pitch_Step;
	s64 Pitch_Factor;
	...
	if ( Reg_PMON & ( 1 << Channel ) & ~1 )
	{
		...
		Pitch_Factor = ((s64)adpcm_decoder::clamp64 ( PreviousSample ));
		Pitch_Factor += 0x8000;
...
void SPU::Run ()
{
	...
	ssX Sample, PreviousSample, ChSampleL, ChSampleR, SampleL, SampleR, CD_SampleL, CD_SampleR, ReverbSampleL, ReverbSampleR;
	ssX FOutputL, FOutputR, ROutputL, ROutputR;
...
//#endif
	...
	if ( pCoreRegs->CTRL >> 15 )
	{
		...
		if ( pCoreRegs->MVOL_L >> 15 )
		{
			...
			VolumeEnvelope ( MVOL_L_Value, MVOL_L_Cycles, pCoreRegs->MVOL_L & 0x7f, ( pCoreRegs->MVOL_L >> 13 ) & 0x3 );
		...
		if ( pCoreRegs->MVOL_R >> 15 )
		{
			...
			VolumeEnvelope ( MVOL_R_Value, MVOL_R_Cycles, pCoreRegs->MVOL_R & 0x7f, ( pCoreRegs->MVOL_R >> 13 ) & 0x3 );
	...
	for ( Channel = 0; Channel < 24; Channel++ )
	{
		...
		if ( pCoreRegs->CTRL >> 15 )
		{
			...
			if ( pChRegs0->VOL_L >> 15 )
			{
				...
				VolumeEnvelope ( VOL_L_Value [ Channel ], VOL_L_Cycles [ Channel ], pChRegs0->VOL_L & 0x7f, ( pChRegs0->VOL_L >> 13 ) & 0x3 );
			...
			if ( pChRegs0->VOL_R >> 15 )
			{
				...
				VolumeEnvelope ( VOL_R_Value [ Channel ], VOL_R_Cycles [ Channel ], pChRegs0->VOL_R & 0x7f, ( pChRegs0->VOL_R >> 13 ) & 0x3 );
	...
	*/
	...
	SampleL = ( SampleL * ( (ssX) ((s16)pCoreRegs->CMVOL_L) ) ) >> c_iVolumeShift;
	SampleR = ( SampleR * ( (ssX) ((s16)pCoreRegs->CMVOL_R) ) ) >> c_iVolumeShift;
	...
	SampleL = ( SampleL * ( (ssX) GlobalVolume ) ) >> c_iVolumeShift;
	SampleR = ( SampleR * ( (ssX) GlobalVolume ) ) >> c_iVolumeShift;

	// mix samples
	Mixer [ ( Mixer_WriteIdx + 0 ) & c_iMixerMask ] = adpcm_decoder::clamp64 ( SampleL );
	Mixer [ ( Mixer_WriteIdx + 1 ) & c_iMixerMask ] = adpcm_decoder::clamp64 ( SampleR );
...
static u64 SPU::DMA_ReadyForRead ( void )
{
	return 1;
}


static u64 SPU::DMA_ReadyForWrite ( void )
{
	return 1;
}
...
static u32 SPU::DMA_ReadBlock ( u32* pMemory, u32 Address, u32 BS )
{
	u32 *Data;
...
static u32 SPU::DMA_WriteBlock ( u32* pMemory, u32 Address, u32 BS )
{
#ifdef INLINE_DEBUG_DMA_WRITE
	debug << "\r\nDMA_Write: ";
	debug << " Cycle#" << dec << *_DebugCycleCount;
...
void SPU::ProcessReverbR ( ssX RightInput )
{
	ssX Rin, Rout;
	ssX t_mLDIFF, t_mRDIFF;
	ssX t_mRSAME, t_mRAPF1, t_mRAPF2;
	
	ssX s_dRSAME = ReadReverbBuffer ( ((u32) *_dRSAME) << 2 );
	ssX s_mRSAME = ReadReverbBuffer ( ( ((u32) *_mRSAME) << 2 ) - 1 );

	ssX s_dLDIFF = ReadReverbBuffer ( ((u32) *_dLDIFF) << 2 );
	
	ssX s_dRDIFF = ReadReverbBuffer ( ((u32) *_dRDIFF) << 2 );
	ssX s_mRDIFF = ReadReverbBuffer ( ( ((u32) *_mRDIFF) << 2 ) - 1 );

	ssX s_mRCOMB1 = ReadReverbBuffer ( ((u32) *_mRCOMB1) << 2 );
	ssX s_mRCOMB2 = ReadReverbBuffer ( ((u32) *_mRCOMB2) << 2 );
	ssX s_mRCOMB3 = ReadReverbBuffer ( ((u32) *_mRCOMB3) << 2 );
	ssX s_mRCOMB4 = ReadReverbBuffer ( ((u32) *_mRCOMB4) << 2 );
	
	ssX s_mRAPF1 = ReadReverbBuffer ( ((u32) *_mRAPF1) << 2 );
	ssX s_mRAPF1_dAPF1 = ReadReverbBuffer ( ( ((u32) *_mRAPF1) - ((u32) *_dAPF1) ) << 2 );
	
	ssX s_mRAPF2 = ReadReverbBuffer ( ((u32) *_mRAPF2) << 2 );
	ssX s_mRAPF2_dAPF2 = ReadReverbBuffer ( ( ((u32) *_mRAPF2) - ((u32) *_dAPF2) ) << 2 );
	
	// input from mixer //
	Rin = ( RightInput * ( (ssX) *_vRIN ) ) >> 15;
	
	// same side reflection //
	//[mRSAME] = (Rin + [dRSAME]*vWALL - [mRSAME-2])*vIIR + [mRSAME-2]  ;R-to-R
	t_mRSAME = ( ( ( Rin + ( ( s_dRSAME * ( (ssX) *_vWALL ) ) >> 15 ) - s_mRSAME ) * ( (ssX) *_vIIR ) ) >> 15 ) + s_mRSAME;
	
	// Different Side Reflection //
	//[mRDIFF] = (Rin + [dLDIFF]*vWALL - [mRDIFF-2])*vIIR + [mRDIFF-2]  ;L-to-R
	t_mRDIFF = ( ( ( Rin + ( ( s_dLDIFF * ( (ssX) *_vWALL ) ) >> 15 ) - s_mRDIFF ) * ( (ssX) *_vIIR ) ) >> 15 ) + s_mRDIFF;
	
	// Early Echo (Comb Filter, with input from buffer) //
	//Rout=vCOMB1*[mRCOMB1]+vCOMB2*[mRCOMB2]+vCOMB3*[mRCOMB3]+vCOMB4*[mRCOMB4]
	Rout = ( ( ( (ssX) *_vCOMB1 ) * s_mRCOMB1 ) + ( ( (ssX) *_vCOMB2 ) * s_mRCOMB2 ) + ( ( (ssX) *_vCOMB3 ) * s_mRCOMB3 ) + ( ( (ssX) *_vCOMB4 ) * s_mRCOMB4 ) ) >> 15;
	
	// Late Reverb APF1 (All Pass Filter 1, with input from COMB) //
	//[mRAPF1]=Rout-vAPF1*[mRAPF1-dAPF1], Rout=[mRAPF1-dAPF1]+[mRAPF1]*vAPF1
	t_mRAPF1 = Rout - ( ( ( (ssX) *_vAPF1 ) * s_mRAPF1_dAPF1 ) >> 15 );
	Rout = s_mRAPF1_dAPF1 + ( ( s_mRAPF1 * ( (ssX) *_vAPF1 ) ) >> 15 );
	
	// Late Reverb APF2 (All Pass Filter 2, with input from APF1) //
	// [mRAPF2]=Rout-vAPF2*[mRAPF2-dAPF2], Rout=[mRAPF2-dAPF2]+[mRAPF2]*vAPF2
	t_mRAPF2 = Rout - ( ( ( (ssX) *_vAPF2 ) * s_mRAPF2_dAPF2 ) >> 15 );
	Rout = s_mRAPF2_dAPF2 + ( ( s_mRAPF2 * ( (ssX) *_vAPF2 ) ) >> 15 );
	
	// Output to Mixer (Output volume multiplied with input from APF2) //
	// RightOutput = Rout*vROUT
	ReverbR_Output = ( Rout * ( (ssX) *_vROUT ) ) >> 15;
...
void SPU::ProcessReverbL ( ssX LeftInput )
{
	ssX Lin, Lout;
	
	// outputs
	ssX t_mLDIFF, t_mRDIFF;
	ssX t_mLSAME, t_mLAPF1, t_mLAPF2;
	
	// inputs
	ssX s_dLSAME = ReadReverbBuffer ( ((u32) *_dLSAME) << 2 );
	ssX s_mLSAME = ReadReverbBuffer ( ( ((u32) *_mLSAME) << 2 ) - 1 );
	
	ssX s_dLDIFF = ReadReverbBuffer ( ((u32) *_dLDIFF) << 2 );
	ssX s_mLDIFF = ReadReverbBuffer ( ( ((u32) *_mLDIFF) << 2 ) - 1 );
	
	ssX s_dRDIFF = ReadReverbBuffer ( ((u32) *_dRDIFF) << 2 );
	
	ssX s_mLCOMB1 = ReadReverbBuffer ( ((u32) *_mLCOMB1) << 2 );
	ssX s_mLCOMB2 = ReadReverbBuffer ( ((u32) *_mLCOMB2) << 2 );
	ssX s_mLCOMB3 = ReadReverbBuffer ( ((u32) *_mLCOMB3) << 2 );
	ssX s_mLCOMB4 = ReadReverbBuffer ( ((u32) *_mLCOMB4) << 2 );
	
	ssX s_mLAPF1 = ReadReverbBuffer ( ((u32) *_mLAPF1) << 2 );
	ssX s_mLAPF1_dAPF1 = ReadReverbBuffer ( ( ((u32) *_mLAPF1) - ((u32) *_dAPF1) ) << 2 );
	
	ssX s_mLAPF2 = ReadReverbBuffer ( ((u32) *_mLAPF2) << 2 );
	ssX s_mLAPF2_dAPF2 = ReadReverbBuffer ( ( ((u32) *_mLAPF2) - ((u32) *_dAPF2) ) << 2 );
	
	// input from mixer //
	Lin = ( LeftInput * ( (ssX) *_vLIN ) ) >> 15;
	
	// same side reflection //
	//[mLSAME] = (Lin + [dLSAME]*vWALL - [mLSAME-2])*vIIR + [mLSAME-2]  ;L-to-L
	t_mLSAME = ( ( ( Lin + ( ( s_dLSAME * ( (ssX) *_vWALL ) ) >> 15 ) - s_mLSAME ) * ( (ssX) *_vIIR ) ) >> 15 ) + s_mLSAME;
	
	// Different Side Reflection //
	//[mLDIFF] = (Lin + [dRDIFF]*vWALL - [mLDIFF-2])*vIIR + [mLDIFF-2]  ;R-to-L
	t_mLDIFF = ( ( ( Lin + ( ( s_dRDIFF * ( (ssX) *_vWALL ) ) >> 15 ) - s_mLDIFF ) * ( (ssX) *_vIIR ) ) >> 15 ) + s_mLDIFF;
	
	// Early Echo (Comb Filter, with input from buffer) //
	//Lout=vCOMB1*[mLCOMB1]+vCOMB2*[mLCOMB2]+vCOMB3*[mLCOMB3]+vCOMB4*[mLCOMB4]
	Lout = ( ( (ssX) *_vCOMB1 ) * s_mLCOMB1 + ( (ssX) *_vCOMB2 ) * s_mLCOMB2 + ( (ssX) *_vCOMB3 ) * s_mLCOMB3 + ( (ssX) *_vCOMB4 ) * s_mLCOMB4 ) >> 15;
	
	// Late Reverb APF1 (All Pass Filter 1, with input from COMB) //
	//[mLAPF1]=Lout-vAPF1*[mLAPF1-dAPF1], Lout=[mLAPF1-dAPF1]+[mLAPF1]*vAPF1
	t_mLAPF1 = Lout - ( ( ( (ssX) *_vAPF1 ) * s_mLAPF1_dAPF1 ) >> 15 );
	Lout = s_mLAPF1_dAPF1 + ( ( s_mLAPF1 * ( (ssX) *_vAPF1 ) ) >> 15 );
	
	// Late Reverb APF2 (All Pass Filter 2, with input from APF1) //
	// [mLAPF2]=Lout-vAPF2*[mLAPF2-dAPF2], Lout=[mLAPF2-dAPF2]+[mLAPF2]*vAPF2
	t_mLAPF2 = Lout - ( ( ( (ssX) *_vAPF2 ) * s_mLAPF2_dAPF2 ) >> 15 );
	Lout = s_mLAPF2_dAPF2 + ( ( s_mLAPF2 * ( (ssX) *_vAPF2 ) ) >> 15 );
	
	// Output to Mixer (Output volume multiplied with input from APF2) //
	// LeftOutput = Lout*vLOUT
	ReverbL_Output = ( Lout * ( (ssX) *_vLOUT ) ) >> 15;
...
ssX SPU::ReadReverbBuffer ( u32 Address )
{
...
void SPU::WriteReverbBuffer ( u32 Address, ssX Value )
{
	...
	// address is ready for use with shift right by 1
	RAM [ Address >> 1 ] = adpcm_decoder::clamp64 ( Value );
}
...
static ssX SPU::Calc_sample_gx ( u32 SampleOffset_Fixed16, ssX Sample0, ssX Sample1, ssX Sample2, ssX Sample3 )
{
	u32 i;
	ssX Output;
	...
	Output = ( ( Sample0 * ((ssX) gx [ i ]) ) + ( Sample1 * ((ssX) gx [0x100 + i]) ) + ( Sample2 * ((ssX) gx [0x1ff - i]) ) + ( Sample3 * ((ssX) gx [0xff - i]) ) ) >> 15;
...
static ssX SPU::Calc_sample_filter ( ssX x0, ssX x1, ssX x2, ssX y1, ssX y2 )
{
	ssX Output;
